...
class ProjectSceneObject_1 extends ClientSceneObject_1 {
    ...
    /**
     * 碰撞事件 onCollision(touchRes:{ isObstacle: boolean, touchSceneObjects: ProjectSceneObject_1[] })
     */
     ...
     * 我的上一次接触者列表
     */
    protected myLastTouchObjects: ProjectSceneObject_1[] = [];
    /**
     * 是否来自恢复存档数据
     */
     ...
     */
    getSaveData(): any {
        var o: any = { myLastTouchObjects: [], moveState: null, recordMoveRoadInfo: this.recordMoveRoadInfo, eventStartWaitInfo: this.eventStartWaitInfo };
     ...
     * @return 对象行为处理器
     */
    addBehavior(behaviorData: any[], loop: boolean, targetSceneObject: ProjectSceneObject_1, onOver: Callback, cover: boolean, startIndex: number = 0, Immediate: boolean = true, forceStopLastBehavior: boolean = false, delayFrame: number = 0, executor: SceneObjectEntity = null) {
        // 设置玩家对象行为前的处理
        ...
        var soBehavior: ProjectSceneObjectBehaviors = new ProjectSceneObjectBehaviors(this, loop, targetSceneObject as any, Callback.New((onOver: Callback, soBehavior: ProjectSceneObjectBehaviors) => {
            // this.behaviors.pop();
            var idx = this.behaviors.indexOf(soBehavior);
    ...
    autoFindRoadMove(toX: number, toY: number, ifObstacleHandleMode: number = 0, costTime: number = 0, useAstar: boolean = true, whenCantMoveRetry: boolean = false, useGridObstacle: boolean = true, forceDir4: boolean = false, fromAutoRetry: boolean = false) {
        ...
        if (this.isJumping) return;
        // 获取目的地
        var currentScene = Game.currentScene as ProjectClientScene;
        var realLineArr: number[][];
        var toP: Point = new Point(toX, toY);
        // 限制在屏幕范围内
        ...
        os.add_ENTERFRAME(() => {
            ...
            if (this.currentJumpFrame > frameTotal) {
                ...
                if (this.inScene) {
                    ...
                    this.event(ProjectSceneObject_1.JUMP_OVER);
                }
                return;
            }
            // 位置偏移
            var per1 = this.currentJumpFrame / frameTotal;
            ...
            if (this.currentJumpFrame <= frameHalf) {
                var per2 = this.currentJumpFrame / frameHalf;
     ...
     * @pparam clacTouchEvent [可选] 默认值=true 是否计算碰触事件模式
     */
    refreshCoordinate(alreadySetTempPosHelper: boolean = false, alreadySetRect: boolean = false, clacTouchEvent: boolean = true) {
        if (!this.inScene) return;
        var scene: ProjectClientScene = this.scene as any;
     ...
     * @param 是否等待成功
     */
    eventStartWait(trigger: ProjectSceneObject_1, faceToTrigger: boolean = true): boolean {
     ...
     */
    clearMyTouchRecord(targetSo: ProjectSceneObject_1 = null): void {
        if (targetSo) {
    ...
    clearTouchMeRecord(): void {
        for (var i = 0; i < Game.currentScene.sceneObjects.length; i++) {
            ...
            if (targetSo instanceof ProjectSceneObject_1) targetSo.clearMyTouchRecord(this);
        }
    }
    /**
     * 是否允许接触
     ...
     * 判断指定对象是否在我的接触者列表中
     */
    isInMyTouchList(targetSo: ProjectSceneObject_1): boolean {
    ...
    private doStartMove(movePath: number[][], costTime: number = 0, useGridObstacle: boolean = false, fromAutoRetry: boolean = false): void {
        // 转为Point数组
        var roadsArr: Point[] = [new Point(this.x, this.y)];
    ...
    protected updateCoordinate(_nowTime: number): void {
        var per: number;
        var isChangeDir: boolean = false;
        var isMoveOver: boolean = false;
        ...
        if (this.nowRoadTime == 0) {
            var thisRoadS: number = this.thisRoadS = thisRoadP.distance(nextRoadP.x, nextRoadP.y);
        ...
        // var nowPClone = new Point(nowP.x,nowP.y);
        // 是否移动过
     ...
     * @return 是否触发过事件
     */
    protected touchEventHandle(touchRes: { isObstacle: boolean, touchSceneObjects: ProjectSceneObject_1[] }): boolean {
        // 恢复存档时当前不会触发事件
        ...
        var subtractList: ProjectSceneObject_1[] = ArrayUtils.compare(touchRes.touchSceneObjects, this.myLastTouchObjects).subtract;
    ...
    protected parallelEventUpdate() {
        ...
        // 未能真正场景完成时忽略执行
        if (GameGate.gateState < GameGate.STATE_4_PLAYER_CONTROL_START) return;
    ...
    private onGamePauseChangeHandle() {
        ...
        for (var i in this.triggerLines) {
            var trigger: CommandTrigger = this.triggerLines[i];
